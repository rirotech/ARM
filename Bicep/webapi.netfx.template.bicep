...
@description('Sets client affinity on the api site (TRUE if in-process sessions are used).')
...
var webAppName = '${department}-${appName}-${environment}-api'
...
resource slotPrivateEndpoint 'Microsoft.Network/privateEndpoints@2022-11-01' = [for (item, i) in webAppSlotNames:if(!empty(privateEndpointSubnetName) && webAppSlotNames[0] != 'emptyList') {
